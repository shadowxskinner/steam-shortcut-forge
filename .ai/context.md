...
- System and vendor `.desktop` files are never edited. Overrides live in the user's XDG applications directory and preserve launcher content except for `Icon=` in `[Desktop Entry]`, the optional `X-KairoHero` data key, and ownership metadata.
- `X-KairoHero` holds an absolute local path to a landscape hero. It is data, not proof of ownership. Authority to overwrite, restore, or delete still comes from the live `X-Kairo-Managed` / `X-ShortcutForge-Managed` marker.
- Heroes did not exist in the legacy product; there is no `X-ShortcutForge-Hero` key.
...
- Tests should use isolated HOME/XDG state where applicable and must never write to the developer's real configuration, icon store, hero store, or applications directory.
